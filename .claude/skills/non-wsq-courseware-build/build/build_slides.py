...
def _topics_by_day():
    ...
    try:
        ...
        # Words shared by several topic titles (e.g. "ITIL", "SERVICE") can't
        # identify a topic — only words unique to one title may act as labels.
        from collections import Counter
        _wc=Counter(w.upper() for tt in C.TOPICS
                    for w in set(re.findall(r"[A-Za-z]{4,}",tt["title"])))
        by_day={}
...
# Show EVERY training day: days are split across the two columns so a 3-day
# course renders Day 1 (+2) on the left and the remaining day(s) on the right.
def _day_block(d):
    return [(f"Day {d} — {C.DAY_THEMES.get(d,'')}",0)]+[_topic_line(_TB[n]) for n in _BY_DAY.get(d,[]) if n in _TB]
_days=sorted(d for d in range(1,C.DAYS+1)) or [1]
_split=(len(_days)+1)//2
_left=[r for d in _days[:_split] for r in _day_block(d)]
_right=[r for d in _days[_split:] for r in _day_block(d)]+[
 ("Daily timing",0),("9:30am–6:30pm · 1-hour lunch · tea breaks within training time",1)]
_lh="Day 1" if _split==1 else f"Days 1–{_days[_split-1]}"
_rh=f"Day {_days[_split]}" if len(_days)-_split==1 else f"Days {_days[_split]}–{_days[-1]}"
two_col(f"Lesson Plan — {C.DAYS} Day{'s' if C.DAYS>1 else ''}, 8 hours/day",_left,_right,
 kicker="SCHEDULE",lhead=_lh,rhead=_rh)
